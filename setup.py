...
class Environment:
    ...
    def _parse_backend_from_cli(self, arg: str) -> None:
        ...
        req_files = sorted(
            [
                os.path.splitext(f)[0].replace("requirements_", "")
    ...
    def _process_arguments(self) -> None:
        ...
        if self.updater:
            get_backend = T.cast(
                "lib_utils",  # type:ignore[attr-defined,valid-type]
                import_module("lib.utils"),
            ).get_backend
...
class RequiredPackages:
    ...
    def _get_missing_python(
        ...
        for req in requirements:
            ...
            if not req.specifier.contains(installed_version):
                logger.debug(
                    "Adding Python package '%s' for specifier change from '%s' to '%s'",
                    package["package"],
                    installed_version,
                    str(req.specifier),
                )
    ...
    def _get_required_conda(self) -> list[dict[T.Literal["package", "channel"], str]]:
        ...
        to_add = (
            _CONDA_BACKEND_REQUIRED.get(self._env.backend, [])
        ...
        for pkg in to_add:
            ...
            logger.debug(
                "Adding conda required package '%s' for system '%s'('%s'))",
                pkg,
                self._env.backend,
    ...
    def _get_missing_conda(
        self,
    ) -> dict[str, list[dict[T.Literal["name", "package"], str]]]:
        ...
        for req, channel in zip(requirements, channels):
            ...
            if exists[-1] != channel:
                logger.debug(
                    "Adding Conda package '%s' for channel change from '%s' to '%s'",
                    package["package"],
                    exists[-1],
                    channel,
                )
            ...
            if not req.specifier.contains(exists[0]):
                logger.debug(
                    "Adding Conda package '%s' for specifier change from '%s' to '%s'",
                    package["package"],
                    exists[0],
                    spec_str,
                )
...
class Checks:  # pylint:disable=too-few-public-methods
    ...
    def _user_input(self) -> None:
        ...
        if self._env.backend is None:
            self._docker_ask_enable()
            self._cuda_ask_enable()
        if not self._env.system.is_linux and (
            self._env.enable_docker and self._env.backend == "nvidia"
        ):
    ...
    def _check_cuda(self) -> None:
        ...
        if cuda.versions:
            ...
            msg = (
                f"Globally installed Cuda version{'s' if len(cuda.versions) > 1 else ''} "
    ...
    def _check_rocm(self) -> None:
        ...
        msg += (
            "The installation can proceed, but you will need to install ROCm version "
...
class Status:
    ...
    def _print_conda(self, line: str) -> None:
        ...
        if info["lib"] not in self._conda_tracked:
            self._conda_tracked[info["lib"]] = {
                "size": self._parse_size(info["tot"]),
                "done": float(info["prg"]),
            }
        else:
        ...
        prog = min(sum(v["done"] for v in self._conda_tracked.values()) / count, 100.0)
    ...
    def _print_pip(self, line: str) -> None:
        ...
        """
        if (
            line.lower().startswith("installing collected packages:")
            and len(line) > self._max_width
        ):
...
class Installer:
    ...
    def __call__(self) -> int:
        ...
        """
        with Popen(self._command, bufsize=0, stdout=PIPE, stderr=PIPE) as proc:
            lines = b""
            while True:
            ...
            if returncode and proc.stderr is not None:
                for line in proc.stderr.readlines():
                    clean = self._seen_line_log(
                        line.decode("utf-8", errors="replace"), is_error=True
                    )
                    if clean:
...
class Install:  # pylint:disable=too-few-public-methods
    ...
    def _from_conda(
        self, packages: list[dict[T.Literal["name", "package"], str]], channel: str
    ) -> None:
        ...
        if not _InstallState.failed:
            if _InstallState.messages:
                for msg in _InstallState.messages:
                    logger.warning(msg)
            logger.info(
                "All Faceswap dependencies are met. You are good to go.\r\n\r\n"
            ...
            if not _InstallState.messages:
                msg += (
                    "This may be temporary and might be fixed by re-running this script. "
                    "Otherwise check 'faceswap_setup.log' to see which failed and install "
                    "these packages manually."
                )
            else:
                msg += (
                    "Further information can be found in 'faceswap_setup.log'. The following "
