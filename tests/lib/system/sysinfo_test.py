...
def test_init(sys_info_instance: _SysInfo) -> None:
    ...
    assert sys_info_instance._python == {
...
def test_properties(sys_info_instance: _SysInfo) -> None:
    ...
    for prop in ints:
        ...
        assert isinstance(getattr(sys_info_instance, prop), int), (
            f"sysinfo property '{prop}' not int"
        )

    for prop in strs:
        assert hasattr(sys_info_instance, prop), f"sysinfo missing property '{prop}'"
        assert isinstance(getattr(sys_info_instance, prop), str), (
            f"sysinfo property '{prop}' not str"
        )
...
def test__configs__parse_configs(
    configs_instance: _Configs, mocker: pytest_mock.MockerFixture
) -> None:
...
def test__state__get_state_file(
