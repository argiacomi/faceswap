...
_LR_STEPS = [(1e-5, 100), (3.4e-6, 250), (9e-4, 599), (6e-5, 1000)]
...
@pytest.mark.parametrize(
    ("target_lr", "steps", "current_step"), _LR_STEPS_CURRENT, ids=_LR_STEPS_CURRENT_IDS
)
def test_set_current_learning_rate(
    model_fixture: Model, target_lr: float, steps: int, current_step: int
) -> None:
...
@pytest.mark.parametrize(
    ("steps", "current_step", "action"), _STEPS_CURRENT, ids=_STEPS_CURRENT_ID
)
def test_output_status(
    ...
    notation_args = [x.args for x in instance._format_notation.call_args_list]  # type:ignore[attr-defined]
...
_STEPS_CURRENT_CALL_ID = [
    f"steps:{x[0]}|current_step:{x[1]}|action:{x[2]}" for x in _STEPS_CURRENT_CALL
]


@pytest.mark.parametrize(
    ("steps", "current_step", "action"), _STEPS_CURRENT_CALL, ids=_STEPS_CURRENT_CALL_ID
)
def test__call__(
    model_fixture: Model,
    steps: int,
