...
def test_ConfigItem_missing_required(group, info, status):
    ...
    default = "test"
    if status.startswith("success"):
        dclass = ConfigItem(datatype=dtype, default=default, group=group, info=info)
        ...
        with pytest.raises(ValueError):
            ConfigItem(datatype=dtype, default=default, group=group, info=info)
...
@pytest.mark.parametrize(
    _FLOAT_SET_PARAMS, _FLOAT_SET_CONFIG, ids=[x[-1] for x in _FLOAT_SET_CONFIG]
)
