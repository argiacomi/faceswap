...
def _view_action(window, label: str):  # type:ignore[no-untyped-def]
    """Return a View menu action by label."""
    for menu_action in window.menuBar().actions():
        if menu_action.text() != "View" or menu_action.menu() is None:
            continue
        for action in menu_action.menu().actions():
            if action.text() == label:
                return action
    raise AssertionError(f"View action not found: {label}")
...
def test_view_menu_actions_follow_runtime_display_visibility(  # type:ignore[no-untyped-def]
    ...
    qtbot.addWidget(window)
    tabs = _display_tabs(window)
    analysis = _view_action(window, "Analysis")
    preview = _view_action(window, "Preview")
    graph = _view_action(window, "Graph")

    assert analysis.isEnabled() is True
    assert preview.isEnabled() is False
    assert graph.isEnabled() is False
    ...
    assert graph.isEnabled() is True
...
def test_job_finished_resets_progress_and_view_actions(  # type:ignore[no-untyped-def]
    ...
    assert _view_action(window, "Preview").isEnabled() is False
    assert _view_action(window, "Graph").isEnabled() is False
