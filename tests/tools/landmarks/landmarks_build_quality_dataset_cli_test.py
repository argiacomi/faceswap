...
def test_scenario_filter_and_samples_per_scenario(tmp_path: Path) -> None:
    ...
    source.write_text(
        json.dumps(
            {
                "samples": [
                    {"sample_id": "occ1", "image": "", "landmarks": _points_68().tolist(), "conditions": {"scenario": "occluded"}},
                    {"sample_id": "occ2", "image": "", "landmarks": _points_68().tolist(), "conditions": {"scenario": "occluded"}},
                    {"sample_id": "clean", "image": "", "landmarks": _points_68().tolist(), "conditions": {"scenario": "clean"}},
                ]
            }
        ),
        encoding="utf-8",
...
def test_manifest_merge_replaces_only_selected_dataset_slice(tmp_path: Path) -> None:
    ...
    first.write_text(
        json.dumps(
            {
                "samples": [
                    {"sample_id": "keep", "image": "", "landmarks": _points_68().tolist(), "conditions": {"scenario": "keep"}},
                    {"sample_id": "old", "image": "", "landmarks": _points_68().tolist(), "conditions": {"scenario": "replace"}},
                ]
            }
        ),
        encoding="utf-8",
    ...
    second.write_text(
        json.dumps(
            {
                "samples": [
                    {"sample_id": "new", "image": "", "landmarks": _points_68().tolist(), "conditions": {"scenario": "replace"}}
                ]
            }
        ),
        encoding="utf-8",
        ...
        json.dumps(
            {
                "dataset": "cofw",
                "samples": [
                    {"sample_id": "a", "dataset": "cofw", "condition": "one", "landmarks": "landmarks.npy", "source": {"dataset": "cofw", "source_id": "same"}},
                    {"sample_id": "b", "dataset": "cofw", "condition": "two", "landmarks": "landmarks.npy", "source": {"dataset": "cofw", "source_id": "same"}},
                ],
            }
        ),
        encoding="utf-8",
