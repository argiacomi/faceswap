...
def test_static_weights_default_output_uses_shared_filename() -> None:
    assert compute_static_weights.DEFAULT_OUTPUT == f"configs/ensemble/{STATIC_WEIGHTS_FILENAME}"


def test_failure_viewer_defaults_use_shared_debug_names() -> None:
    assert failure_viewer.DEFAULT_DEBUG_DIR == f"outputs/landmark_{DEBUG_DIRNAME}"
...
def test_production_gate_outputs_use_shared_filenames() -> None:
    ...
    assert production_promotion_gate.PRODUCTION_POLICY_FAILURES_CSV == PRODUCTION_POLICY_FAILURES_CSV
