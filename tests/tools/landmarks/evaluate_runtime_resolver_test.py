...
def test_roll_aware_veto_vetoes_roll_outlier_and_picks_consensus_survivor() -> None:
    ...
    assert decision.chosen == "spiga"
    assert decision.vetoed == ("static_weighted",)
    assert decision.consensus_roll_deg == pytest.approx(3.0)


def test_roll_aware_veto_falls_back_when_every_candidate_lacks_pose() -> None:
    candidates = [_candidate("a"), _candidate("b")]
    metrics = {"a": _metric(0.02, roll=None), "b": _metric(0.04, roll=None)}
...
def test_aggregate_reports_summarizes_empty_input() -> None:
    assert aggregate_reports([], policy="roll_aware_veto", failure_threshold=0.08) == {
