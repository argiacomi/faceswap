...
class LearningRateWarmup:
    ...
    def _output_status(self) -> None:
        ...
        if self._current_step == 1:
            logger.info(
                "[Learning Rate Warmup] Start: %s, Target: %s, Steps: %s",
        ...
        if self._current_step == self._steps:
            print()
            logger.info(
                "[Learning Rate Warmup] Final Learning Rate: %s",
                self._format_notation(self._target_lr),
            )
            return
        ...
        if self._current_step in self._reporting_points:
            ...
            progress = int(
                round(
                    100
                    / (len(self._reporting_points) - 1)
                    * self._reporting_points.index(self._current_step),
                    0,
                )
            )
            logger.info(
                "[Learning Rate Warmup] Step: %s/%s (%s), Current: %s, Target: %s",
                self._current_step,
                self._steps,
                f"{progress}%",
                self._format_notation(self._current_lr),
