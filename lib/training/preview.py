...
class Samples:
    ...
    def __init__(
        self, coverage_ratio: float, has_mask: bool, mask_opacity: int, mask_color: str
    ) -> None:
        ...
        self._mask_color_array = (
            np.array(hex_to_rgb(mask_color), dtype=np.float32)[..., 2::-1] / 255.0
        ).astype(np.float32)
    ...
    def __repr__(self) -> str:
        ...
        params = ", ".join(
            ...
            if k in ("_coverage_ratio", "_has_mask", "_mask_opacity", "_mask_color")
        )
    ...
    def _get_background(
        self, targets: npt.NDArray[np.float32], patch_size: int, padding: int
        ...
        logger.debug(
            "[%s] Created background display patches: %s",
            self._name,
            format_array(retval),
        )
        return retval
    ...
    def _get_foreground(
        ...
        if self._coverage_ratio == 1.0:
            ...
            retval[:, 0] = targets[
                :, :, padding : patch_size - padding, padding : patch_size - padding, :3
            ]

        logger.debug(
            "[%s] Created foreground display patches: %s",
            self._name,
            format_array(retval),
        )
        return retval
    ...
    def _get_headers(
        self,
        num_swaps: int,
        patch_width: int,  # pylint:disable=too-many-locals
        ...
        """
        labels = [
            get_label(i, num_swaps)
            + (f" > {get_label(i + j, num_swaps, next_identity=True)}" if j > 0 else "")
    ...
    def _create_image(self, patches: npt.NDArray[np.float32]) -> npt.NDArray[np.uint8]:
        ...
        images = (
            patches.transpose(2, 3, 0, 1, 4, 5).reshape(
                (rows * identities, cols * src_side * img_count, channels)
            )
            * 255.0
        ).astype(np.uint8)
    ...
    def get_preview(
        self, predictions: npt.NDArray[np.float32], targets: npt.NDArray[np.float32]
    ) -> npt.NDArray[np.uint8]:
