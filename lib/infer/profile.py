...
class ModelProfile:
    ...
    def _output_stats(self) -> None:
        ...
        vram_alloc_str = [
            "N/A" if i < 0 else str(int(round(i / (1024 * 1024)))) for i in self.vram[0]
        ]
        vram_res_str = [
            "N/A" if i < 0 else str(int(round(i / (1024 * 1024)))) for i in self.vram[1]
        ]
...
class DataTracker:  # pylint:disable=too-many-instance-attributes
    ...
    def get_samples(
        self, index: int | None = None, adjusted: bool = False
...
class Profiler:
    ...
    def __init__(self, runner: ExtractRunner) -> None:
        ...
        self._chain = T.cast(
            "list[ExtractRunner[ExtractHandler]]",  # For intellisense purposes
            get_pipeline(runner),
        )
    ...
    def _check_for_torch(self, plugin: ExtractPlugin) -> bool:
        ...
        logger.debug(
            "[Profiler] Scanning for torch Module: %s(%s)",
            plugin.name,
    ...
    def _update_config_file(cls, plugins: list[ExtractPlugin]):
        ...
        for plugin_name, plugin in zip(f_names, plugins, strict=False):
            ...
            if old_val == new_val:
                logger.debug(
                    "[Profiler] Skipping unchanged batch size %s for '%s'",
                    old_val,
                    plugin_name,
                )
                continue
            logger.debug(
                "[Profiler] Updating batch size from %s to %s for '%s'",
                old_val,
                new_val,
                plugin_name,
            )
