...
class Converter:  # pylint:disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        self._face_scale = 1.0 - arguments.face_scale / 100.0
    ...
    def reinitialize(self) -> None:
        ...
        self._face_scale = 1.0 - self._args.face_scale / 100.0
        self._adjustments = Adjustments()
    ...
    def _patch_image(self, predicted: ConvertItem) -> np.ndarray | list[bytes]:
        ...
        if self._writer_pre_encode is None:
            ...
            if self.cli_arguments.writer == "patch":
                ...
                kwargs["matrices"] = np.array(
                    [
                        self._get_warp_matrix(
                            face.adjusted_matrix, patched_face.shape[1], face.y_offset
                        )
    ...
    def _warp_to_frame(self, reference: AlignedFace, face: np.ndarray, frame: np.ndarray) -> None:
        ...
        foreground, mask = np.split(
            frame_face,  # pylint:disable=unbalanced-tuple-unpacking
            (3,),
            axis=-1,
        )
        background *= 1.0 - mask
    ...
    def _post_warp_adjustments(self, background: np.ndarray, new_image: np.ndarray) -> np.ndarray:
        ...
        if self._draw_transparent:
            ...
            foreground, mask = np.split(
                new_image,  # pylint:disable=unbalanced-tuple-unpacking
                (3,),
                axis=-1,
            )
            foreground *= mask
