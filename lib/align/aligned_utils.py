...
def get_adjusted_center(
    ...
    offset = target_offset - source_offset - [0.0, y_offset]
    offset *= source_size
...
@T.overload
def sub_crop(
    image: npt.NDArray[np.uint8], offset: npt.NDArray[np.int32], out_size: int
...
def sub_crop(
    image: npt.NDArray[np.uint8 | np.float32],  # pylint:disable=too-many-locals
...
def batch_adjust_matrices(
    matrices: npt.NDArray[np.float32], size: int, padding: int, reverse: bool = False
) -> npt.NDArray[np.float32]:
...
def batch_sub_crop(
    images: npt.NDArray[np.uint8 | np.float32],
    offsets: npt.NDArray[np.int32],
    out_size: int,
    base_grid: tuple[npt.NDArray[np.int32], npt.NDArray[np.int32]] | None = None,
) -> npt.NDArray[np.uint8 | np.float32]:
...
def batch_align(
    images: list[npt.NDArray[ImageDTypeT]],  # pylint:disable=too-many-locals
    image_ids: npt.NDArray[np.int32],
    matrices: npt.NDArray[np.float32],
    size: int,
    fast_upscale: bool = True,
...
def batch_resize(
    images: npt.NDArray[ImageDTypeT], size: int, fast_upscale: bool = True
) -> npt.NDArray[ImageDTypeT]:
...
def points_to_68(
    landmarks: npt.NDArray[np.float32], landmark_type: LandmarkType | None = None
) -> npt.NDArray[np.float32]:
