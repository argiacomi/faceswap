...
class Conv2DBlock:  # pylint:disable=too-many-instance-attributes
    ...
    def _get_layers(self) -> list[layers.Layer]:
        ...
        conv: layers.Layer = DepthwiseConv2D if self._use_depthwise else Conv2D  # pyright:ignore[reportAssignmentType]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._conv = Conv2DBlock(
            filters * scale_factor * scale_factor,
            kernel_size,
            strides=(1, 1),
            padding=padding,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._fast or (not self._fast and self._filters > 0):
            ...
            self._upsample = layers.UpSampling2D(
...
class UpscaleResizeImagesBlock:
    ...
    def __init__(
        ...
        self._resize = KResizeImages(
            size=scale_factor, interpolation=interpolation, name=f"{name}_resize"
        )
        self._conv = Conv2D(
            filters,
            kernel_size,
        ...
        self._conv_trans = layers.Conv2DTranspose(
            filters, 3, strides=2, padding=padding, name=f"{name}_convtrans"
        )
...
class UpscaleDNYBlock:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._upsample = layers.UpSampling2D(
            size=size, interpolation=interpolation, name=f"{name}_upsample2d"
        )
        self._convs = [
            Conv2DBlock(
                filters,
...
class ResidualBlock:
    ...
    def _get_layers(self) -> list[layers.Layer]:
        ...
        retval.append(
            Conv2D(
                self._filters,  # pyright:ignore[reportArgumentType]
