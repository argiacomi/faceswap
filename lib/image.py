...
def read_image(
    ...
    with_metadata: T.Literal[False] = False,
) -> npt.NDArray[np.uint8] | None: ...


@T.overload
def read_image(
    filename: str, raise_error: T.Literal[True], with_metadata: T.Literal[False] = False
...
def read_image(
    ...
    raise_error: T.Literal[False] = False,
    *,
    with_metadata: T.Literal[True],
) -> tuple[npt.NDArray[np.uint8], PNGHeader]: ...


@T.overload
def read_image(
    filename: str, raise_error: T.Literal[True], with_metadata: T.Literal[True]
) -> npt.NDArray[np.uint8]: ...


def read_image(
    filename: str,  # noqa: C901  # pylint:disable=too-many-statements,too-many-branches
    ...
    try:
        ...
        if np.issubdtype(image.dtype, np.integer):
            ...
            image = (np.clip(image, 0.0, 1.0) * 255.0).astype(np.float32)
...
def read_image_batch(
    filenames: list[str], with_metadata: T.Literal[False] = False
) -> np.ndarray: ...


@T.overload
def read_image_batch(
    filenames: list[str], with_metadata: T.Literal[True]
) -> tuple[np.ndarray, list[PNGHeader]]: ...


def read_image_batch(
    filenames: list[str], with_metadata: bool = False
    ...
    with futures.ThreadPoolExecutor() as executor:
        images = {
            executor.submit(  # NOTE submit strips positionals, breaking type-checking
                read_image,  # type:ignore[arg-type]
                filename,
                raise_error=True,  # pyright:ignore[reportArgumentType]
                with_metadata=with_metadata,
            ): idx  # pyright:ignore[reportArgumentType]
...
def encode_image(
    ...
    metadata: PNGHeader | dict[str, T.Any] | bytes | None = None,
) -> bytes:
    """Encode an image.
...
def tiff_write_meta(
    image: bytes,  # pylint:disable=too-many-locals
    data: PNGHeader | dict[str, T.Any] | bytes,
) -> bytes:
        ...
        while True:
            if self._error_state.has_error:
                ...
                if current is main_thread():
                    ...
                    logger.debug(
                        "[%s.%s] Thread error detected in worker thread",
                        current.name,
                        self._name,
                    )
                    break
...
class SingleFrameLoader(ImagesLoader):
    ...
    def video_meta_data(
        ...
        if self._reader is None:
            return None
        return {
            "pts_time": self._reader.info.pts.tolist(),
...
class ImagesSaver(ImageIO):
    ...
    def save(
        self, filename: str, image: bytes | np.ndarray, sub_folder: str | None = None
    ) -> None:
        ...
        if self._error_state is not None and self._error_state.has_error:
            logger.debug(
                "[%s.%s] Thread error detected in worker thread. Not putting",
                current_thread().name,
                self._name,
            )
            return
