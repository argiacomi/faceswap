...
class PreviewExtract:
    ...
    def _place_previews(self, frame_dims: tuple[int, int]) -> Image.Image | None:
        ...
        display = np.vstack(
            [
                np.hstack(T.cast("Sequence", samples[row * cols : (row + 1) * cols]))
...
class Images:
    ...
    def delete_preview(self) -> None:
        ...
        for item in os.listdir(self._path_preview):
            if item.startswith(os.path.splitext(TRAINING_PREVIEW)[0]) and item.endswith(
                (".jpg", ".png")
            ):
...
class PreviewTrigger:
    ...
    def __init__(self) -> None:
        logger.debug("Initializing: %s", self.__class__.__name__)
        self._trigger_files = {
            "update": os.path.join(PATH_CACHE, ".preview_trigger"),
