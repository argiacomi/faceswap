...
def resolve_dataset_source(
    ...
    *,
    ...
    if url is None and spec.google_drive_file_id is None:
        hint = f" {spec.manual_hint}" if spec.manual_hint else ""
        raise FileNotFoundError(
            f"{spec.dataset} source not found in {cache_root / spec.cache_root_name} and no download source is configured.{hint}"  # noqa: E501
        )
    ...
    try:
        ...
        Exception
    ) as err:  # pragma: no cover - exercised by adapter-level tests with monkeypatches
        hint = f" {spec.manual_hint}" if spec.manual_hint else ""
