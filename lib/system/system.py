...
class System:  # pylint:disable=too-many-instance-attributes
    ...
    def __init__(self) -> None:
        ...
        self.is_conda = "conda" in sys.version.lower() or os.path.exists(
    ...
    def validate_python(self, max_version: tuple[int, int] | None = None) -> bool:
        ...
        retval = (
            VALID_PYTHON[0] <= sys.version_info[:2] <= max_python
            and self.python_architecture == "64bit"
        )
        logger.debug(
            "Python version %s(%s) within %s - %s(64bit): %s",
            self.python_version,
        ...
        if not retval:
            print()
            logger.error(
                "Your Python version %s(%s) is unsupported. Please run with Python "
            ...
            print()
            logger.error(
                "If you have recently upgraded faceswap, then you will need to create a "
                "new virtual environment."
            )
            logger.error(
                "The easiest way to do this is to run the latest version of the Faceswap "
                "installer from:"
            )
...
class Packages:
    ...
    def installed_conda(self) -> dict[str, tuple[str, str, str]]:
        ...
        installed = [
            re.sub(" +", " ", line.strip())
            for line in self._installed_conda
