...
class Inference:
    ...
    def _get_valid_inputs_for_layer(self, layer) -> list[keras.Layer]:
        ...
        retval: list[keras.Layer] = [
            t._keras_history.operation  # pylint:disable=protected-access
            for t in side_tensors
        ]
        logger.debug("[Inference] '%s' valid inputs: %s", layer.name, retval)
        return retval

    def _get_valid_layer_inputs(
        ...
        """
        retval = {
            layer: self._get_valid_inputs_for_layer(layer)
            for layer in T.cast(list[keras.Layer], model.layers)
            if not isinstance(layer, keras.layers.InputLayer)
        }
        logger.debug(
            "[Inference] '%s' layer valid inputs for side index %s: %s",
            model.name,
            self._side_idx,
    ...
    def _get_output_layer(self, model: keras.models.Model) -> keras.Layer:
        ...
        """
        history: list[node.KerasHistory] = [
            t._keras_history  # pylint:disable=protected-access
        ...
        logger.debug(
            "[Inference] '%s' outputs count: %s, output layer count: %s",
            model.name,
            outputs_count,
            layer_count,
        )
        ...
        logger.debug(
            "[Inference] '%s' output layer for side index %s: '%s'",
            model.name,
            self._side_idx,
            retval.name,
        )
    ...
    def _backwards_recurse(
        self, layer: keras.Layer, seen: set[keras.Layer] | None = None
    ) -> list[keras.Layer]:
        ...
        logger.debug(
            "[Inference] Got inputs for '%s': %s",
            layer.name,
            [n.name for n in next_layers],
        )
