...
import numpy as np
import torch
from keras import Variable, layers
...
if T.TYPE_CHECKING:
    ...
    from keras.optimizers import Optimizer as K_Optimizer
...
def get_all_sub_models(
    model: k_models.Model, models: list[k_models.Model] | None = None
) -> list[k_models.Model]:
...
class IO:
    ...
    def multiple_models_in_folder(self) -> list[str] | None:
        ...
        logger.debug(
            "[IO] plugin name: %s, plugins: %s, test result: %s, retval: %s",
            self._plugin.name,
            plugins,
            test,
            retval,
    ...
    def _update_legacy(self) -> None:
        ...
        if self.model_exists:
            logger.debug(
                "[IO] Existing model file is current: '%s'", os.path.basename(self.filename)
            )
            return
    ...
    def _save_optimizer(self, optimizer: Optimizer) -> None:
        ...
        with zipfile.ZipFile(self.filename, "a") as z_file:
            z_file.writestr(
                "optimizer.pt", opt_bytes, compress_type=zipfile.ZIP_DEFLATED, compresslevel=1
            )
    ...
    def _should_backup(self, save_average: float) -> bool:
        ...
        if backup:  # Update lowest loss values to the state file
            self._plugin.state.lowest_avg_loss = save_average
            logger.debug(
                "[IO] Updated lowest historical save iteration average from: %s to: %s",
                old_average,
                save_average,
            )
    ...
    def _maybe_backup(self) -> tuple[float, bool]:
        ...
        if not save_average or not should_backup:
            logger.debug(
                "[IO] Not backing up model (save_average: %s, should_backup: %s)",
                save_average,
                should_backup,
            )
...
class Weights:
    ...
    def load(self, model_exists: bool) -> None:
        ...
        if model_exists and self._weights_file:
            logger.warning(
                "Ignoring weights file '%s' as this model is resuming.", self._weights_file
            )
            return
        ...
        if loaded_ops == 0:
            raise FaceswapError(
                f"No weights were successfully loaded from your weights file: "
    ...
    def _get_weights_model(self) -> list[k_models.Model]:
        ...
        retval = get_all_sub_models(
            k_models.load_model(  # pyright:ignore[reportArgumentType]
    ...
    def _load_layer_weights(
        ...
        if old_weights[0].shape != new_weights[0].shape:
            logger.warning(
                "The weights for layer '%s' are of incompatible shapes. Skipping.", layer.name
            )
            return 0
...
class OptimizerMigrate:
    ...
    def _get_optimizer_and_group_ids(
        self,
    ) -> tuple[K_Optimizer, dict[int, T.Literal["decay", "no_decay"]]] | None:
    ...
    def _build_optimizer_state(
        self, optimizer: K_Optimizer, decay_indices: list[int], no_decay_indices: list[int]
        ...
        logger.debug(
            "[OptimizerMigrate] mapping for '%s': %s -> %s", optimizer.name, mapping[0], mapping[1]
        )
        ...
        vars_ = {
            mapping[1][idx]: getattr(optimizer, x)._value.data
            for idx, x in enumerate(mapping[0])
            if isinstance(getattr(optimizer, x), Variable)
        }
        weights = {
            x: getattr(optimizer, mapping[0][idx])
        ...
        for dst_idx, src_idx in enumerate(ordered):
            ...
            if not all(hasattr(v, "_value") for v in layer.values()):
                logger.debug(
                    "[OptimizerMigrate] Skipping variable without torch param: %s",
    ...
    def _get_parameter_groups(
        cls, optimizer: K_Optimizer, weight_indices: list[int], bias_indices: list[int]
    ) -> list[dict[str, T.Any]]:
    ...
    @classmethod
    def _get_scaler_state(
        cls, optimizer: LossScaleOptimizer | None
        ...
        if (
            not hasattr(optimizer, "dynamic_growth_steps")
        ...
        retval = {
            "scale": float(optimizer.dynamic_scale.numpy()),
            "growth_factor": 2.0,
            "backoff_factor": 0.5,
    ...
    def convert(self) -> dict[str, T.Any] | None:
        ...
        if hasattr(optimizer, "inner_optimizer"):
            logger.debug(
                "[OptimizerMigrate] Extracting inner optimizer %s from %s",
