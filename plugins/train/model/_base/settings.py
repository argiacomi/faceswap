...
class Optimizer:
    ...
    def _configure_clipping(
        ...
        if method in clip_types:
            ...
            logger.debug(
                "Setting clipping kwargs for '%s': %s",
                method,
                {k: v for k, v in self._kwargs.items() if k == clip_types[method]},
            )
            return
        ...
        assert hasattr(self._optimizer, "_clip_gradients"), (
            "keras.BaseOptimizer._clip_gradients no longer exists"
        )
    ...
    def _configure(self) -> None:
        ...
        self._configure_clipping(
            T.cast(
                T.Literal["autoclip", "norm", "value", "none"],
...
class Settings:
    ...
    def loss_scale_optimizer(
        cls, optimizer: optimizers.Optimizer
    ...
    def _set_keras_mixed_precision(cls, enable: bool) -> None:
        ...
        logger.debug(
            "%s mixed precision. (Compute dtype: %s, variable_dtype: %s)",
    ...
    def _get_mixed_precision_layers(self, layers: list[dict]) -> list[str]:
        ...
        for layer in layers:
            ...
            if "dtype" not in config:
                logger.debug(
                    "Skipping unsupported layer: %s %s",
                    layer.get("name", f"class_name: {layer['class_name']}"),
                    config,
                )
                continue
