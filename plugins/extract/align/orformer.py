...
def _get_checkpoint_paths(model_config: ORFormerModelConfig) -> dict[str, str]:
    ...
    hgnet_path = _extract_checkpoint(archive_path, cache_dir, model_config.hgnet)
    orformer_path = _extract_checkpoint(archive_path, cache_dir, model_config.orformer)
    return {"hgnet": hgnet_path, "orformer": orformer_path}


def _extract_checkpoint(archive_path: str, cache_dir: str, checkpoint: ORFormerCheckpoint) -> str:
    ...
    if os.path.exists(output_path) and _hash_file(output_path) == checkpoint.sha256:
        return output_path
    ...
    with (
        ...
        archive.open(checkpoint.archive_member) as source,
