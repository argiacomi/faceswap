...
class SPIGA(nn.Module):
    def __init__(self, num_landmarks=98, num_edges=15, steps=3, **kwargs):
        ...
        self.swindow = (
            0.25  # Scale of the cropped window at first step (Dft. 25% w.r.t the input featuremap)
        )
        ...
        diagonal_mask = (
            torch.ones(self.num_landmarks, self.num_landmarks) - torch.eye(self.num_landmarks)
        ).type(torch.bool)
        ...
        self.gcn = nn.ModuleList(
            [StepRegressor(self.embedded_dim, 256, self.nstack) for i in range(self.steps)]
        )
    ...
    def extract_visual_embedded(self, pts_proj, receptive_field, step):
        ...
        # Crop windows. MPS does not support border padding for grid_sample, so clamp to valid
        # feature-map pixel centers and use zero padding there.
        padding_mode = "border"
        if receptive_field.device.type == "mps":
            padding_mode = "zeros"
            edge = 1.0 / receptive_field.shape[-1]
            grid = grid.clamp(min=-1.0 + edge, max=1.0 - edge)
        crops = torch.nn.functional.grid_sample(
            receptive_field, grid, padding_mode=padding_mode, align_corners=False
        )  # BxCxLxK*K
