...
Reviewed through the current Manual Tool follow-up work after #119/#121, #101/#104/#105/#106/#111, the #107 GUI regressions, the edit-triggered filter-refresh patch (`Manual Tool: refresh filters after editable changes`), logger handler cleanup, persistence startup/save race stabilization, and a reported green full test suite.
...
| Filter-aware face panel | Current frame panel reflects the selected filtered frame. | Full-session filtered grid remains #108. | ✅ / #108 for grid |

**Ticket:** #107 is implemented, covered by GUI regressions, and closed. The future cross-frame filtered grid remains #108.
...
- Keep open until #108/#112 are closed or explicitly deferred and Qt/fallback launch smoke is complete.
