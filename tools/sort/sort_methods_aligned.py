...
class SortAlignedMetric(SortMethod):
    ...
    def score_image(
        ...
        if (
            not self._logged_lm_count_once
            and face.landmark_type == LandmarkType.LM_2D_4
            and self.__class__.__name__ != "SortSize"
        ):
            logger.warning(
                "You have selected to sort by an aligned metric, but at least one face "
    ...
    def binning(self) -> list[list[str]]:
        ...
        for i in tqdm(range(0, img_list_len - 1), desc="Grouping", file=sys.stdout, leave=False):
            ...
            for key, references in reference_groups.items():
                try:
                    score = self._get_avg_score(
                        fl1,  # pyright:ignore[reportArgumentType]
                        references,
                    )
                except TypeError:
                    score = float("inf")
