    ...
    def update_config(self) -> None:
        ...
        for section, options in self.tk_vars.items():
            for option_name, tk_option in options.items():
                ...
                logger.debug(
                    "Updating config: '%s', '%s' from %s to %s",
                    section,
                    option_name,
                    repr(old_value),
                    repr(new_value),
...
class ActionFrame(ttk.Frame):  # pylint:disable=too-many-ancestors
    ...
    def __init__(self, app: Preview, parent: ttk.Frame) -> None:
        ...
        defaults = {
            opt: self._format_to_display(val) if opt != "face_scale" else val
            for opt, val in self._options.items()
        }
        self._busy_bar = self._build_frame(
            defaults,
    ...
    def convert_args(self) -> dict[str, T.Any]:
        ...
        retval = {
            opt if opt != "color" else "color_adjustment": self._format_from_display(
                self._tk_vars[opt].get()
            )
...
class ConfigFrame(ttk.Frame):  # pylint:disable=too-many-ancestors
    ...
    def __init__(self, parent: OptionsBook, config_key: str, options: dict[str, T.Any]):
