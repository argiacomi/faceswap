...
class Mask(Editor):
    ...
    def _add_controls(self) -> None:
        ...
        """
        masks = sorted(msk.title() for msk in self._det_faces.available_masks)
        # ``available_masks`` is empty when the alignments file holds no faces yet, and contains
        # only ``"None"`` when faces exist but none have masks. Prefer a real mask name when one
        # is present; otherwise fall back to the placeholder so the radio group can still render.
        real_masks = [mask for mask in masks if mask != "None"]
        if real_masks:
            default = real_masks[0]
        else:
            if not masks:
                masks = ["None"]
            default = masks[0]
    ...
    def _set_full_frame_meta(
        ...
        scaled_mask_roi = np.rint(mask.original_roi * self._globals.current_frame.scale).astype(
            "int32"
        )
    ...
    def _get_cursor_shape(
        ...
        y_2: int = 0,
        outline: str = "black",
        state: T.Literal["normal", "hidden", "disabled"] = "hidden",
    ) -> int:
