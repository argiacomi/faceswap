...
class Viewport:
    ...
    def mesh_kwargs(self) -> dict[T.Literal["polygon", "line"], dict[str, T.Any]]:
        ...
        return {
            "polygon": {"fill": "", "outline": color, "state": state},
    ...
    def _get_tk_face_object(
        self, face: DetectedFace, image: np.ndarray, is_active: bool
    ) -> TKFace:
...
class Recycler:
    ...
    def __init__(self, canvas: FacesViewer) -> None:
        self._canvas = canvas
        self._assets: dict[T.Literal["image", "line", "polygon"], list[int]] = {
            "image": [],
            "line": [],
            "polygon": [],
        }
        self._mesh_methods: dict[T.Literal["line", "polygon"], T.Callable] = {
...
class VisibleObjects:
    ...
    def _add_rows(self, existing_rows: int, required_rows: int) -> None:
        ...
        for row in range(existing_rows, required_rows):
            ...
            meshes.append(
                [
                    {} if face is None else self._recycler.get_mesh(face)
