...
def _detector_class(module_name: str) -> type[ExtractPlugin]:
    ...
    if len(candidates) > 1:
        logger.debug(
            "Multiple detector classes found in %s; using %s",
            module.__name__,
...
def _prepare_detector_batch(
    detector: ExtractPlugin, image_path: Path
...
def _boxes_and_scores(detector: ExtractPlugin, batch: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    scores = (
        boxes[:, 4].astype("float32")
        if boxes.shape[1] >= 5
        else np.ones((boxes.shape[0],), dtype="float32")
    )
