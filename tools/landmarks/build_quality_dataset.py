...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--scenarios",
        default=None,
        help="Comma-separated condition/scenario groups to include. If omitted, all dataset-provided conditions are kept.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="replace",
        help="Replace manifest contents or merge this dataset/scenario slice into an existing manifest.",
    )
    parser.add_argument(
        "--allow-overlap",
        action="store_true",
    ...
    parser.add_argument(
        "--audit-only",
        action="store_true",
        help="Validate and rewrite dataset_audit.json for an existing manifest without regenerating data.",
    )
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.audit_only:
        if any((args.wflw_annotations, args.cofw_json, args.source_dir, args.source_zip)):
            raise ValueError(
                "--audit-only only accepts --dataset, --output-dir, --allow-overlap, and logging options"
            )
