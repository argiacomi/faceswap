...
def _build_model_adapters(models: T.Sequence[str], *, device: str) -> dict[str, LandmarkAdapter]:
    ...
    adapters = {
        model: build_landmark_adapter(model, device=device, input_is_rgb=False, input_scale=(0, 255))
        for model in models
    }
    ...
    for model in models:
        ...
        for start in range(0, len(items), batch_size):
            ...
            if len(predictions) != len(batch_items):
                raise ValueError(
                    f"adapter '{model}' returned {len(predictions)} predictions for batch of {len(batch_items)}"
                )
            for item, prediction in zip(batch_items, predictions, strict=True):
                ...
                if canonical.coordinate_space != "frame":
                    raise ValueError(
                        f"adapter '{model}' returned coordinate space '{canonical.coordinate_space}', expected frame"
                    )
                cache.write(
