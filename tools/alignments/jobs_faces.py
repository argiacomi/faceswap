...
class FromFaces:
    ...
    def _sort_alignments(
        self,
        alignments: dict[str, dict[str, list[tuple[int, FileAlignments, str, PNGSource]]]],
        ...
        for fname, frames in alignments.items():
            ...
            for frame in tqdm(sorted(frames), desc=f"Sorting {fname}", leave=False):
                ...
                for real_idx, (f_id, aln, f_path, f_src) in enumerate(
...
class RemoveFaces:
    ...
    def _update_png_headers(self) -> None:
        ...
        to_update = [  # Items whose face index has changed
            x for x in src if x[1].face_index != items[x[1].source_filename].index(x[1].face_index)
        ]
...
class FaceToFile:
    ...
    def _check_and_update(self, alignment: PNGAlignments, face: FileAlignments) -> None:
